...
export async function GET(req: Request) {
  ...
  switch (type) {
    case "parties": {
      ...
      const rows = data.map((p) => [p.name, p.type, p.gstin || "", p.phone || "", p.email || "", p.city || "", p.state || "", p.openingBalance]);
    ...
    case "items": {
      ...
      const rows = data.map((i) => [i.name, i.sku || "", i.hsn || "", i.barcode || "", i.unit, i.salePrice, i.purchasePrice, i.gstRate, i.currentStock]);
      csv = toCSV(headers, rows);
      filename = "items.csv";
      break;
    }
    case "invoices": {
      ...
      const rows = data.map((i) => [i.number, new Date(i.date).toLocaleDateString("en-IN"), i.party.name, i.party.gstin || "", i.subTotal, i.taxTotal, i.grandTotal, i.status, i.amountPaid]);
      csv = toCSV(headers, rows);
    ...
    case "purchases": {
      ...
      const rows = data.map((p) => [p.number, new Date(p.date).toLocaleDateString("en-IN"), p.party.name, p.party.gstin || "", p.subTotal, p.taxTotal, p.grandTotal, p.status]);
      csv = toCSV(headers, rows);
    ...
    case "payments": {
      ...
      const rows = data.map((p) => [p.number, new Date(p.date).toLocaleDateString("en-IN"), p.party.name, p.type, p.mode, p.amount, p.reference || ""]);
      csv = toCSV(headers, rows);
